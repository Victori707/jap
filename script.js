...
let isDevMode = false;
const RATE_JPY_TO_RUB = 0.68; // курс покупки для пересчёта из ен в рубли
let _lastComputedRubTotal = null; // для анимации изменения суммы
...
function setDevMode(state) {
  ...
  if (devBar) {
    ...
    let savePlanBtn = document.getElementById('save-plan-btn');
    ...
    if (isDevMode && !savePlanBtn) {
      savePlanBtn = document.createElement('button');
      savePlanBtn.id = 'save-plan-btn';
      savePlanBtn.textContent = '📝 Сохранить план';
      savePlanBtn.style.cssText = 'margin-left: 10px; padding: 8px 16px; background: #6A5ACD; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 14px;';
      savePlanBtn.onclick = persistDataToServer;
      devBar.appendChild(savePlanBtn);
    }
    if (!isDevMode) {
      if (saveBtn) saveBtn.remove();
      if (savePlanBtn) savePlanBtn.remove();
    }
  }
...
async function saveHTMLToFile() {
  try {
    ...
    // Пытаемся сохранить через сервер
    try {
      ...
      // пробуем считать как JSON, если не JSON — возьмём текст для диагностики
      ...
      if (contentType.includes('application/json')) {
        ...
        // не JSON — считаем успешным, но без деталей
        result = { ok: true, filename: 'index_saved.html' };
      }
      if (result.ok) {
      ...
      // Фоллбэк: локальная выгрузка файла напрямую из браузера
      ...
      alert('⚠️ Серверное сохранение не удалось, файл загружен локально как index_saved.html.\n\nПодробности: ' + (srvErr && srvErr.message ? srvErr.message : srvErr));
    }
...
function renderItinerary(days) {
  ...
  days.forEach((day, idx) => {
    ...
    const expensesBlock = isDevMode
      ...
      : (currency === 'JPY'
        ? `<div class="day-summary">Итоговые траты: <b>${formatRub(expVal)} ¥</b> (~${formatRub(Math.round(expVal * RATE_JPY_TO_RUB))} ₽ по курсу 0,68)</div>`
    ...
    if (isDevMode) {
      ...
      // Показываем карту только если она действительно доступна и есть координаты
      const hasAnyCoords = Array.isArray(day.timeline) && day.timeline.some(ev => ev.location && ev.location.lat && ev.location.lng);
      const canShowGMap = typeof window !== 'undefined' && window.google && window.google.maps;
      mediaBlock = (hasAnyCoords && canShowGMap) ? `<div id="map-day-${day.id}" class="map-container"></div>` : '';
    }
...
<li class="timeline-event editing" data-eidx="${eidx}" data-day-idx="${idx}">
  ...
  <button class="dev-photo-delete" data-del-evt="${eidx}" data-day-idx="${idx}">–</button>
    ...
    const contentHtml = `
      ...
      <div class="day-content">
        <div class="timeline-col"><ul class="timeline-list">${timelineHtml}</ul></div>
        ${mediaBlock ? `<div class="map-col">${mediaBlock}</div>` : ``}
      </div>
    `;
    daySection.innerHTML = contentHtml;
  ...
  // каждый раз после перерисовки — обновляем общий подсчёт
  computeAndRenderTotals();
}
...
// Подсчёт общих сумм: считает по дням и выводит в footer
function computeAndRenderTotals() {
  const el = document.getElementById('computed-total');
  if (!el) return;
  let totalRub = 0;
  let totalJpy = 0;
  let totalRubNative = 0;
  (daysData || []).forEach(d => {
    const val = coerceExpense(d.expenses);
    const cur = (d.currency || 'RUB').toUpperCase();
    if (cur === 'JPY') {
      totalJpy += val;
      totalRub += val * RATE_JPY_TO_RUB;
    } else {
      totalRubNative += val;
      totalRub += val;
    }
  });
  const roundedRub = Math.round(totalRub);
  el.textContent = `~${formatRub(roundedRub)} ₽ (из них ${formatRub(Math.round(totalJpy))} ¥ ≈ ${formatRub(Math.round(totalJpy * RATE_JPY_TO_RUB))} ₽; RUB напрямую: ${formatRub(totalRubNative)} ₽)`;
  // Подсветка изменений
  if (_lastComputedRubTotal !== null && _lastComputedRubTotal !== roundedRub) {
    el.classList.remove('pulse');
    // reflow trick для рестарта анимации
    // eslint-disable-next-line no-unused-expressions
    void el.offsetWidth;
    el.classList.add('pulse');
  }
  _lastComputedRubTotal = roundedRub;
}

// Сохранение текущего плана (данных) на сервер в data/itinerary.json
async function persistDataToServer() {
  try {
    const btn = document.getElementById('save-plan-btn');
    if (btn) { btn.disabled = true; btn.textContent = '⏳ Сохраняю план...'; }
    // очищаем derived поля и убеждаемся, что expenses — число
    const clean = JSON.parse(JSON.stringify(tripDataRaw || { days: [] }));
    clean.days = (clean.days || []).map(d => ({
      ...d,
      expenses: coerceExpense(d.expenses)
    }));
    ...
    alert('✅ План сохранён в data/itinerary.json');
  } catch (e) {
    alert('❌ Не удалось сохранить план на сервер: ' + (e?.message || e));
  } finally {
    const btn = document.getElementById('save-plan-btn');
